...
manifests, and resolve any issues that result from linting before proceeding::
    ...
    sudo sh -c "WORKSPACE=~/treasuremap/deployment_files ~/pegleg/tools/pegleg.sh lint -p /workspace"

Next, use pegleg to perform the merge that will yield the combined global +
site type + site YAML::

    mkdir -p ~/${NEW_SITE}_yaml
    sudo sh -c "WORKSPACE=~/treasuremap/deployment_files ~/pegleg/tools/pegleg.sh site -p /workspace collect $NEW_SITE > ~/${NEW_SITE}_yaml/$NEW_SITE.yaml"
...
render from Pegleg will not be used as input in subseuqnet steps, but is useful
...
and addressing any Deckhand errors prior to submitting via Shipyard::
    ...
    sudo sh -c "WORKSPACE=~/treasuremap/deployment_files ~/pegleg/tools/pegleg.sh site -p /workspace render $NEW_SITE"
