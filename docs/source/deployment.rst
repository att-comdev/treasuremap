...
Move the secrets to your NewSite's location for passphrase secrets::

    mv secrets_tools/*.yaml site/NewSite/secrets/passphrases

Public SSH keys for environment access are stored under
``site/NewSite/secrets/publickey/``. Make copies of ``ca846m_ssh_public_key.yaml``
