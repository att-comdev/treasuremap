...
File containing site-level settings for Ceph. Ceph is deploy on control plane
...
2. Ceph disk mapping and disk layout is the same across Ceph nodes (i.e. only
   one control plane host profile, or multiple profiles with the same disk
   related paramters)
3. If doing a fresh install, disk are unlabeled or not labeled from a previous
   Ceph install, so that Ceph chart will not fail
...
Then, the data section of this file will look similar to the following (journal
mountpoints coming from the host profile)::
